...
export default function Support({ tickets = [], supportReady = true, supportSchemaWarning = null }) {
    const { t } = useI18n();
            ...
            <div className="ciete-page max-w-4xl">
                ...
                <div className="rounded-xl border border-border bg-surface p-5 lg:p-6">
                    <div className="mb-5 grid gap-5 xl:grid-cols-[minmax(0,15rem)_minmax(0,1fr)]">
                        ...
                        <div className="flex justify-end pt-2">
                            <button
                                type="submit"
                                disabled={processing || !supportReady}
                ...
                <div className="rounded-xl border border-border bg-surface">
                    <div className="border-b border-border px-5 py-4">
                        <h3 className="text-sm font-semibold text-text-main">{t('supportPage.myTickets')}</h3>
                    </div>

                    {tickets.length === 0 ? (
                        <p className="px-5 py-6 text-sm text-text-hint">{t('supportPage.noTicketsYet')}</p>
                    ) : (
                        <div className="divide-y divide-border">
                            {tickets.map((ticket) => (
                                <div key={ticket.id_solicitud_soporte} className="flex flex-col gap-3 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                                    <div className="min-w-0 space-y-2">
                                        <div className="flex flex-wrap items-center gap-2">
                                            <p className="text-sm font-semibold text-text-main">
                                                #{ticket.id_solicitud_soporte} · {ticket.asunto}
                                            </p>
                                            <StatusBadge status={ticket.estado} t={t} />
                                            <PriorityBadge priority={ticket.prioridad} t={t} />
                                        </div>
                                        <p className="text-xs text-text-hint">{t(`supportPage.topics.${ticket.tema}`)}</p>
                                        <p className="text-xs text-text-muted">
                                            {t('supportPage.lastUpdate')}:{' '}
                                    ...
                                    <div className="shrink-0">
                                        <Link
                                            href={route('support.show', ticket.id_solicitud_soporte)}
                                            className="inline-flex items-center rounded-lg border border-border bg-surface-2 px-3 py-2 text-xs font-bold uppercase tracking-widest text-text-main transition hover:bg-surface"
                                        >
                                            {t('supportPage.openTicket')}
                                        </Link>
                                    </div>
                                </div>
                            ))}
